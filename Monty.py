...
class Monty:
    ...
    def metropolis(self,replica):
        ...
        thisenergy = self.energy(replica.chain) + self.restraint.energy(replica.chain)  # + self.novelty_dist(replica.chain)
    ...
    def novelty_dist(self, chain):
        contacts = chain.contactstate()
        ...
        if len(contact_set) == 0:
            ...
            for prev_contact in self.novelty_archive:
                same_contacts = contact_set.union(prev_contact)
                all_distances.append(len(same_contacts))
            ...
            if novelty_score > self.archive_threshold:
                self.novelty_archive.append(contact_set)
